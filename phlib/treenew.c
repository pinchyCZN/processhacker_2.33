    ...
    )
{
    if (!Node->s.CachedColorValid)
    {
		...
		getNodeColor.BackColor = GetSysColor(COLOR_WINDOW);
		getNodeColor.ForeColor = GetSysColor(COLOR_WINDOWTEXT);
    ...
    {
		if (PhGetColorBrightness(Node->s.DrawBackColor) - PhGetColorBrightness(Node->s.DrawForeColor) < 150) // slightly less than half
			Node->s.DrawForeColor = Node->s.DrawForeColor ^ 0xFFFFFF;
    }
}
